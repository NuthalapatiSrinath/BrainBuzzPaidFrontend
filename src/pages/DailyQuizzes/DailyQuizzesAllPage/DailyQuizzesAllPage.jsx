...
import styles from "./DailyQuizzesAllPage.module.css";
...
// Helper function to group by category (like in AllLiveClassesPage)
...
export default function DailyQuizzesAllPage() {
  ...
  useEffect(() => {
    const flattened = [];
    DAILY_QUIZZES.categories.forEach((parentCat) => {
      const subList = DAILY_QUIZZES.subcategories[parentCat.key] || [];
      subList.forEach((sub) => {
        flattened.push({
          id: `${parentCat.key}-${sub.id}`,
          title: sub.title,
          logo: sub.logo || parentCat.logo, // Use sub-logo, fallback to parent
          path: sub.path || `/dailyquizzes/${parentCat.key}/${sub.id}`,
          category: parentCat.title, // The parent category name for grouping
          description: sub.description || "", // ✅ THIS LINE IS NOW CORRECT
        });
      });
    });
  ...
  const filteredAndGrouped = useMemo(() => {
    ...
    const filtered = allSubcategories.filter((item) => {
      // Check if category matches the active pill
      const categoryMatch =
      ...
      const searchMatch =
        ...
        (item.description || "").toLowerCase().includes(lowerSearchTerm); // Search description
  ...
  return (
    <div className={styles.pageWrapper}>
      {/* Use CategoryHeader, no hero image */}
      <CategoryHeader title="All Daily Quizzes" />

      {/* Filter Bar */}
      <div className={styles.filterBar}>
        <SearchBar
          placeholder="Search for a quiz subcategory..."
              ...
              <div className={styles.grid}>
                {filteredAndGrouped[categoryName].map((item) => (
                  <CategoryCard
                    ...
                    description={item.description} // ✅ DESCRIPTION IS NOW PASSED
