...
import styles from "./EbooksAllPage.module.css";
...
export default function EbooksAllPage() {
  ...
  useEffect(() => {
    const flattened = [];
    EBOOKS_DATA.categories.forEach((parentCat) => {
      const subList = EBOOKS_DATA.subcategories[parentCat.key] || [];
      subList.forEach((sub) => {
        flattened.push({
          id: `${parentCat.key}-${sub.id}`,
          title: sub.title,
          logo: sub.logo || parentCat.logo,
          path: sub.path || `/ebooks/${parentCat.key}/${sub.id}`,
          category: parentCat.title, // The parent category name for grouping
          description: sub.description || "", // Get description
        });
      });
    });
  ...
  return (
    <div className={styles.pageWrapper}>
      <CategoryHeader title="All E-Book Publications" />

      {/* Filter Bar */}
      <div className={styles.filterBar}>
        <SearchBar
          placeholder="Search for an e-book subcategory..."
      ...
      <div className={styles.contentArea}>
        {Object.keys(filteredAndGrouped).length === 0 ? (
          <p className={styles.noResults}>
            No e-books found matching your filters.
          </p>
        ) : (
